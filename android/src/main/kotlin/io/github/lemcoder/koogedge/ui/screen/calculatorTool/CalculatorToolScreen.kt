...
@Composable
fun CalculatorToolScreen(state: CalculatorToolState, onEvent: (CalculatorToolEvent) -> Unit) {
    Column(
        ...
        OutlinedTextField(
            ...
            placeholder = { Text(stringResource(R.string.enter_expression)) },
            modifier = Modifier.fillMaxWidth(),
        )
        ...
        if (state.toolCalls.isEmpty() || state.isCalculating) {
            ...
            LazyColumn(modifier = Modifier.fillMaxWidth().weight(1f).padding(vertical = 8.dp)) {
                items(state.toolCalls) { Text("Tool call: $it") }
        ...
        Button(
            onClick = { onEvent(CalculatorToolEvent.Calculate(expression)) },
